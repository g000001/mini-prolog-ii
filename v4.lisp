...
(in-syntax *mini-prolog-ii-readtable*)


(defvar *mini-prolog-ii-directory*
  (asdf:system-source-directory (asdf:find-system :mini-prolog-ii)))


(defun load-startup-file ()
  (load (merge-pathnames #p"mlg.Start"
                         *mini-prolog-ii-directory*)))

(defun mini-Prolog-II ()			; to run it
  (let ((*readtable* *mini-prolog-ii-readtable*)
        (*package* (find-package :mini-prolog-ii)))
    (banner)
    (format t "~A~%" (load-startup-file))
    (myloop (read_prompt))))
